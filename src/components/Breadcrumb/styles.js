...
import { removerAcentos } from '~/assets/js/auxFunctions';
...
export function SearchButton({ route }) {
  return (
    <a href={`/${removerAcentos(route)}/buscar`} className="btn text-info">
...
export function AddButton({ route }) {
  return (
    <a
      href={`/${removerAcentos(route)}/adicionar`}
...
export function RemoveButton({ route, id }) {
  return (
    <a
      href={`/${removerAcentos(route)}/remover/${id}`}
...
export function GoBackButton({ route }) {
  return (
    <a href={`/${removerAcentos(route)}/listar`} className="btn text-info">
